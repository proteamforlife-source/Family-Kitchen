...
function switchTab(id) {
  tabs.forEach(function(t) {
    el('pg-'+t).classList.remove('on');
    el('tb-'+t).classList.remove('on');
  });
...
function init() {
  db.ref('members').once('value', function(snap) {
    ...
    if (sn && members[sn]) {
      userName = sn; userColor = sc;
      el('authScreen').classList.add('h');
  ...
  db.ref('recipes').on('value', function(snap) {
    recipes = []; testRecipes = [];
    snap.forEach(function(c) { var v = c.val(); if (v.testing) testRecipes.push(v); else recipes.push(v); });
    recipes.reverse(); testRecipes.reverse();
    renderRecipes(); buildCatFilter(); buildTagFilter(); renderTestRecipes();
    if (el('pg-d').classList.contains('on') && userName) renderDashboard();
  });
  db.ref('events').on('value', function(snap) {
    events = []; snap.forEach(function(c) { events.push(c.val()); });
    renderEvents();
    if (el('pg-d').classList.contains('on') && userName) renderDashboard();
  });
  db.ref('shopping').on('value', function(snap) {
    shopItems = []; snap.forEach(function(c) { shopItems.push(c.val()); });
    renderShopping();
  });
  db.ref('bills').on('value', function(snap) {
    bills = []; snap.forEach(function(c) { bills.push(c.val()); });
    renderBills();
    if (el('pg-d').classList.contains('on') && userName) renderDashboard();
  });
...
var plannermodal = t.closest('[data-plannermodal]'); if (plannermodal) { if (!userName) return; var mname = plannermodal.dataset.mname; el('plannerModTitle').textContent = 'Add "' + mname + '" to Planner'; var list = el('plannerDateList'); list.innerHTML = ''; for (var i = 0; i < 14; i++) { var d = new Date(); d.setDate(d.getDate() + i); var label = i === 0 ? 'Today' : i === 1 ? 'Tomorrow' : d.toLocaleDateString('en-AU', { weekday: 'short', day: 'numeric', month: 'short' }); var dk = dKey(d); list.innerHTML += '<button class="planner-date-btn" data-plannerdate="' + dk + '" data-mname="' + esc(mname) + '">' + label + '</button>'; } el('plannerMod').classList.remove('h'); return; }
var plannerdate = t.closest('[data-plannerdate]'); if (plannerdate) { if (!userName) return; var date = plannerdate.dataset.plannerdate; var mname = plannerdate.dataset.mname; var slot = el('plannerMealType').value; var d = new Date(date + 'T00:00:00'); var dayIdx = d.getDay() - 1; if (dayIdx < 0) dayIdx = 6; var monday = new Date(d); var dow = monday.getDay() || 7; monday.setDate(monday.getDate() - dow + 1); var wk = dKey(monday); var mid = 'm' + Date.now(); var notes = el('plannerModNotes').value.trim(); var meal = { id: mid, name: mname, votes: {}, cooker: '', by: userName }; if (notes) meal.notes = notes; db.ref('planner/' + wk + '/' + dayIdx + '/' + slot + '/' + mid).set(meal); el('plannerMod').classList.add('h'); el('plannerModNotes').value = ''; return; }
