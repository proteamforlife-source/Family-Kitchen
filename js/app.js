...
var dqa = t.closest('[data-dqa]'); if (dqa) { if (!userName) return; var ans = dqa.dataset.dqa; if (ans === 'clear') { db.ref('dinnerQ/'+todayKey()+'/'+userName).remove(); } else { db.ref('dinnerQ/'+todayKey()+'/'+userName).set(ans); } setTimeout(renderDashboard, 400); return; }  var st2 = t.closest('[data-switchtab]'); if (st2 && !t.closest('[data-addmeal]') && !t.closest('[data-quickdinner]')) { switchTab(st2.dataset.switchtab); return; }
